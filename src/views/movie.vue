  ...
  <div class="main">
    ...
    >
      ...
      <div class="info">
        ...
        <p v-if="credits.crew[0]">
          Producer:
          <span class="producers" :key="c.id" v-for="c in credits.crew">
            <router-link v-if="c.job === 'Director'" :to=" '/person/' + c.id">{{c.name + " "}}</router-link>
          </span>
        </p>
        <p v-if="info.production_countries[0]">
          Production in
          <span :key="c.name" v-for="c in info.production_countries">{{c.name}}</span>
        </p>
        <p v-if="info.spoken_languages[0]">
          Spoken languages:
          <span
            :key="lang.name"
            v-for="lang in info.spoken_languages"
          >{{lang.name}}</span>
        </p>
        <p
          v-if="info.release_date[0]"
        >Released in {{new Date(info.release_date).toLocaleDateString('en-EN', options) }}</p>
        <p v-if="info.runtime">Runtime: {{info.runtime}} minutes</p>
        <p
          v-if="info.budget"
        >Budget: {{new Intl.NumberFormat('us-US', { style: 'currency', currency: 'USD'}).format(info.budget)}} ({{new Intl.NumberFormat('us-US', { style: 'currency', currency: 'USD'}).format(info.budget / info.runtime) }} per minute)</p>
        <p
          v-if="info.revenue != 0 || null"
        >Revenue: {{new Intl.NumberFormat('us-US', { style: 'currency', currency: 'USD'}).format(info.revenue)}}</p>
